...
function status() { 
  var ss= SpreadsheetApp.getActiveSpreadsheet();
  var statussSheet = ss.getSheetByName("EMPLOYEES"); 
  var getLastRow = statussSheet.getLastRow();  
  return statussSheet.getRange(2,2,3).getValues();  
}
...
function clockIn(employee,status) {
  ...
  {
    // CHECK CLOCK IN
    if(employee ==  mainSheet.getRange(j, 1).getValue() && mainSheet.getRange(j,3).getValue() == '' && status ==  mainSheet.getRange(j, 1).getValue() && mainSheet.getRange(j,3).getValue() == '' )
    {
      error = 'Need to Clock Out before Clocking In';
      return_array.push([error, return_date, employee,status]);
      return return_array;
    }
  ...
  .setNumberFormat("MM/dd/yyyy hh:mm AM/PM")
  .setHorizontalAlignment("center")
  .setFontSize(12);
  mainSheet.getRange(lastRow+1,5).setValue(status)
  .setFontSize(12);
